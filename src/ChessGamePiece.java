...
public abstract class ChessGamePiece {
     ...
     * @param skipMoveGeneration if true, moves will not be generated in the
     * constructor
    ...
    public boolean move(ChessGameBoard board, int row, int col) {
        if (canMove(board, row, col)) {
            ...
            if (isEnemy(board, row, col)) {
                int currentPlayer = ((ChessPanel) board.getParent()).getGameEngine().getCurrentPlayer();
                ChessGraveyard graveyard = ((ChessPanel) board.getParent()).getGraveyard(currentPlayer == 1 ? 2 : 1);
